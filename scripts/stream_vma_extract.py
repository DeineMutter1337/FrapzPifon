#!/usr/bin/env python3
import argparse, hashlib, os, struct, urllib.request
...
def main():
    ...
    req = urllib.request.Request(args.url, headers={'User-Agent':'FrapzPifon-Analyzer/1.0'})
    with urllib.request.urlopen(req, timeout=60) as response:
        reader = zstd.ZstdDecompressor().stream_reader(response)
        first = exact(reader, 12288)
        if first[:4] != b'VMA\0': raise RuntimeError('not a VMA stream')
        ...
        config_names = [int.from_bytes(header[2044+i*4:2048+i*4], 'big') for i in range(256)]
        config_data  = [int.from_bytes(header[3068+i*4:3072+i*4], 'big') for i in range(256)]
        for no, do in zip(config_names, config_data):
            if not no or no not in blobs or do not in blobs: continue
            name = blobs[no].split(b'\0',1)[0].decode('utf-8','replace')
            if '/' in name or '\\' in name: continue
            open(os.path.join(args.destination, name), 'wb').write(blobs[do])
        ...
        for dev_id in range(256):
            ...
            if not size or name_off not in blobs: continue
        ...
        while True:
            eh = reader.read(512)
            if not eh: break
            if len(eh) != 512: raise EOFError('truncated extent header')
            if eh[:4] != b'VMAE': raise RuntimeError(f'bad extent magic at extent {extents}')
            if eh[8:24] != uuid: raise RuntimeError('extent UUID mismatch')
            expected = eh[24:40]
            chk = bytearray(eh); chk[24:40] = b'\0'*16
            if hashlib.md5(chk).digest() != expected: raise RuntimeError('extent checksum mismatch')
            ...
            for i in range(59):
                ...
                if dev_id not in handles: raise RuntimeError(f'unknown device {dev_id}')
                f = handles[dev_id]
                f.seek(cluster_no * CLUSTER)
                for bit in range(16):
                    if mask & (1 << bit): f.write(exact(reader, 4096))
                    else: f.seek(4096, os.SEEK_CUR)
            extents += 1
            if extents % 1000 == 0: print(f'processed {extents} extents')

        for dev_id, f in handles.items():
            f.truncate(devices[dev_id][1]); f.close()
